...
def main():
  activities = ['Prediction', 'EDA', 'Data Visualisation', 'model', 'About us']
  ...
  #     st.dataframe(df.head(10))
    ...
    def add_parameter(name_of_cls):
      ...
      if name_of_cls=="KNN":
        ...
        leaf_size=st.sidebar.slider("leaf_size", 1, 100, key=30)
...
The maximum number of passes over the training data (aka epochs). It only impacts the behavior in the fit method, and not the partial_fit method.<br>    
	""", unsafe_allow_html=True)
  	...
  	image =  Image.open("AU1.png")
  	st.image(image, use_column_width=True)

  # option = st.selectbox('How would you like to be contacted?',('Email', 'Home phone', 'Mobile phone'))
  # st.write('You selected:', option)
  # if option=="Email":
  #   st.write("lazola.javu@gmail.com")
